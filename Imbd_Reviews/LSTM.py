...
train_data,test_data,new_data = get_imbd_reviews(size_train=10000,size_test=10000,seq_size=None)

model = tf.keras.Sequential()
model.add(tf.keras.layers.Embedding(input_dim=vocab_size,output_dim=embed_size,input_length=seq_size))
model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(embed_size)))
...
hist = model.fit(train_data[0],train_data[1],epochs=5,validation_data=test_data,batch_size=64)

model.save("LSTM_imdb_reviews.h5")
with open("LSTM_imdb_reviews.pkl","wb") as f:
